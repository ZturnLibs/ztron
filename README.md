...
Final spike: **64 deterministic checks, all pass** (`FULL_OK`, clean `exit 0`; `WIN_EVENT_OK`/`WIN_QUERY2_OK` are key-window bonuses on top). The second window is created + destroyed for real via `WebviewWindow` (P6.3).

## Tests

```
pnpm test       # 56 tests: 55 pass / 1 skip (surface + unit + core)
