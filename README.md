...
Final spike: **68 deterministic checks, all pass** (`FULL_OK`, clean `exit 0`; `WIN_EVENT_OK`/`WIN_QUERY2_OK` are key-window bonuses on top). The second window is created + destroyed for real via `WebviewWindow` (P6.3).

## Tests

```
pnpm test       # 58 tests: 57 pass / 1 skip (surface + unit + core)
