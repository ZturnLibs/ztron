/**
 * Ztron Svelte Demo 后端：注册演示涉及的插件 + svelte-demo:* 命令。
 * 与 examples/vue-demo/src/main.ts 同构（差异：报告命令为
 * svelte-demo:report，冒烟 tag 为 SVELTE_DEMO_OK，品牌串全部换成 svelte-demo）。
 */
import {
  AppBuilder,
  fsPlugin,
...
console.log(
  `[svelte-demo] backend connected${devUrl ? `, frontend ${devUrl}` : " (inline html)"}`,
);
...
<html>
  <body style="font-family:system-ui;background:#0a0c10;color:#e6eaf2;display:grid;place-content:center;height:100vh;margin:0">
    <div style="text-align:center"><h1>Ztron Svelte Demo</h1><p>请通过 ztron dev 启动（需 Vite 前端）</p></div>
  </body>
</html>`;
...
new AppBuilder(runtime, "com.ztron.svelte-demo")
  ...
  .plugin(pathPlugin({ appId: "com.ztron.svelte-demo" }))
  .plugin(
    shellPlugin({
      scope: [
        { program: "echo", args: ["*"] },
  ...
  .plugin(autostartPlugin({ id: "com.ztron.svelte-demo" }))
  .plugin(
    windowStatePlugin({
      file: `${tjs.tmpDir}/ztron_svelte_demo_window_state.json`,
      restoreOnStartup: false,
    }),
  )
  .plugin(singleInstancePlugin({ identifier: "com.ztron.svelte-demo" }))
  ...
  .setup((app) => {
      ...
      reloadTimer = setInterval(async () => {
        try {
          ...
          if (stamp !== last) {
            ...
            console.log("[svelte-demo] frontend changed -> page reloaded");
          }
        } catch {
    ...
    // 前端冒烟上报：ztron check --expect SVELTE_DEMO_OK 解析这行日志。
    ...
    app.command("svelte-demo:report", (_args, ctx) => {
      const { received } = _args as { received?: string };
      console.log(`[svelte-demo] frontend reported: "${received}"`);
      if (
        tjs.env.ZTRON_CHECK === "1" &&
        received?.split(":")[0] === "SVELTE_DEMO_OK"
      ) {
    ...
    app.command("svelte-demo:emit-ticks", async () => {
      for (let i = 1; i <= 3; i++) {
        await new Promise((r) => setTimeout(r, 120));
        app.emit("svelte-demo:tick", { n: i });
      }
      return "started";
    });

    // Channel 演示：向通道推 1..8（与 vue-demo 的 vue-demo:stream 同模式，
    // 同步发送，结束后 end 关闭通道）。
    app.command("svelte-demo:stream", (args, ctx) => {
