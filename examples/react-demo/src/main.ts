/**
 * Ztron React Demo 后端：注册演示涉及的插件 + react-demo:* 命令。
 * 结构与 examples/showcase 同构（差异：无本地回声服务器，报告命令为
 * react-demo:report，冒烟 tag 为 REACT_DEMO_OK）。
 */
import {
  AppBuilder,
  fsPlugin,
...
console.log(
  `[react-demo] backend connected${devUrl ? `, frontend ${devUrl}` : " (inline html)"}`,
);
...
<html>
  <body style="font-family:system-ui;background:#0a0c10;color:#e6eaf2;display:grid;place-content:center;height:100vh;margin:0">
    <div style="text-align:center"><h1>Ztron React Demo</h1><p>请通过 ztron dev 启动（需 Vite 前端）</p></div>
  </body>
</html>`;
...
new AppBuilder(runtime, "com.ztron.react-demo")
  ...
  .plugin(pathPlugin({ appId: "com.ztron.react-demo" }))
  .plugin(
    shellPlugin({
      scope: [
        { program: "echo", args: ["*"] },
  ...
  .plugin(autostartPlugin({ id: "com.ztron.react-demo" }))
  .plugin(
    windowStatePlugin({
      file: `${tjs.tmpDir}/ztron_react_demo_window_state.json`,
      restoreOnStartup: false,
    }),
  )
  .plugin(singleInstancePlugin({ identifier: "com.ztron.react-demo" }))
  ...
  .setup((app) => {
      ...
      reloadTimer = setInterval(async () => {
        try {
          ...
          if (stamp !== last) {
            ...
            console.log("[react-demo] frontend changed -> page reloaded");
          }
        } catch {
    ...
    // 前端冒烟上报：ztron check --expect REACT_DEMO_OK 解析这行日志。
    ...
    app.command("react-demo:report", (_args, ctx) => {
      const { received } = _args as { received?: string };
      console.log(`[react-demo] frontend reported: "${received}"`);
      if (
        tjs.env.ZTRON_CHECK === "1" &&
        received?.split(":")[0] === "REACT_DEMO_OK"
      ) {
    ...
    app.command("react-demo:emit-ticks", async () => {
      for (let i = 1; i <= 3; i++) {
        await new Promise((r) => setTimeout(r, 120));
        app.emit("react-demo:tick", { n: i });
      }
      return "started";
    });

    // Channel 演示：向通道推 1..8（与 showcase 的 showcase:stream 同模式，
    // 同步发送，结束后 end 关闭通道）。
    app.command("react-demo:stream", (args, ctx) => {
