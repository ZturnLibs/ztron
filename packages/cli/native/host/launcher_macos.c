...
int main(void) {
  ...
  *slash = '\0';
  ...
  /* forward the declarative config + capabilities the backend reads at
     boot (Resources/ztron.conf.json, Resources/capabilities/) — without
     them the packaged app silently loses every declared window */
  char *env_conf = NULL;
  char conf_path[4200];
  snprintf(conf_path, sizeof(conf_path), "%s/ztron.conf.json", res);
  ...
  if (cf) {
    ...
    if (n > 0 && n < 1024 * 1024) {
      ...
      if (buf && fread(buf, 1, (size_t)n, cf) == (size_t)n) {
        buf[n] = '\0';
        size_t len = sizeof("ZTRON_CONF=") + (size_t)n;
        env_conf = malloc(len);
        if (env_conf) snprintf(env_conf, len, "ZTRON_CONF=%s", buf);
      }
      free(buf);
    }
    fclose(cf);
  ...
  char *env_caps = NULL;
  char caps_path[4200];
  snprintf(caps_path, sizeof(caps_path), "%s/capabilities", res);
  struct stat st;
  if (stat(caps_path, &st) == 0 && S_ISDIR(st.st_mode)) {
    size_t len = sizeof("ZTRON_CAPABILITIES_DIR=") + strlen(caps_path);
    env_caps = malloc(len);
    if (env_caps) snprintf(env_caps, len, "ZTRON_CAPABILITIES_DIR=%s", caps_path);
  }
  ...
  be_env[ei++] = env_key;
  be_env[ei++] = env_url;
  if (env_conf) be_env[ei++] = env_conf;
  if (env_caps) be_env[ei++] = env_caps;
  be_env[ei] = NULL;
  int status = 0;
  pid_t be_pid;
  if (posix_spawn(&be_pid, backend_bin, NULL, NULL, be_argv, be_env) == 0) {
