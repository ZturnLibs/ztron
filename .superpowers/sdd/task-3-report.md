...
- **examples.md**: table rows use real package names from `examples/*/package.json` (`@ztron/example-hello`, `@ztron/example-multiwin`, `@ztron/example-menuprobe`); run commands match README Quick start (`pnpm --filter … dev`). multiwin claims tied to `MULTI_WINDOW_OK` / `SECOND_WINDOW_OK label=second`; hello windows[] description matches `examples/hello/ztron.conf.json` (labels main/conf-second, `url: "frontend"`, inline `html`).
