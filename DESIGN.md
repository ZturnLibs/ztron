...
- **教训**:①GitHub Packages scope 必须与仓库 owner 完全一致(@ztronlib→@ztron,403 排查后重命名五层)②headless macos runner 无 WindowServer 会话,GUI spike 挂窗(native/单测不受影响)→ CI 降级为 spike best-effort + 本地为 GUI 事实源 ③**macOS runner 10×计费**,六轮全链调试烧穿免费额度——macos job 改 workflow_dispatch 手动触发;此教训应更早意识到(每轮全链 ~25 分钟)④CI 上 webview 构建需 Doxygen/clang-format/clang-tidy/py(amalgamation),按需 WEBVIEW_ENABLE_CHECKS=OFF / BUILD_DOCS=OFF
