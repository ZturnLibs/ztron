...
| 层                      | 文件                                                                                       | 覆盖                                                                               | 运行                                          |
| ----------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | --------------------------------------------- |
| **Surface(完整性)**     | `unit/surface.test.ts` + `helpers/manifest.ts`                                             | 框架注册的命令 == 清单;@zturnlibs/ztron-api 导出 == 清单(无缺失/无多余)                      | `pnpm test`                                   |
...
| **Integration(运行时)** | `examples/hello` spike(86 项确定性,含 key-window/真拖放 bonus;EXIT 0)                          | 真实 host+webview 端到端;tjs:` 模块 / 网络 / 会退出应用的命令                      | 手动 `pnpm --filter @zturnlibs/ztron-example-hello dev` |
| **Integration(多窗口)** | `examples/multiwin`(P6.3)                                                             | 运行时第二窗口:建窗→label 路由→窗口 ops→destroy→注册表清理                              | 手动 `pnpm --filter @zturnlibs/ztron-example-multiwin dev` |
