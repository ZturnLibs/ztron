...
| **Integration(运行时)** | `examples/hello` spike(51 项)                                                              | 真实 host+webview 端到端;tjs:` 模块 / 网络 / 会退出应用的命令                      | 手动 `pnpm --filter @ztron/example-hello dev` |
...
```bash
pnpm test          # surface + unit + 原 core.test.ts(49 pass / 1 skip)
