...
features:
  - title: 双进程架构
    details: 原生宿主（窗口/托盘/菜单）与 tjs 异步后端通过 TCP/JSON 协作；@zturnlibs/core 负责 IPC、事件、插件与 ACL。
  - title: Tauri v2 能力对齐
    details: "@zturnlibs/api 移植 @tauri-apps/api；invoke / 事件 / Channel / fs / http / os / store / log / shell / updater 等插件齐备，从 Tauri 迁移有对照指南（见指南）。"
  - title: macOS 全链路已验证
